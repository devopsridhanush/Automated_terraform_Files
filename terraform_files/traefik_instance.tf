...
resource "null_resource" "install_packer" {
  ...
  provisioner "local-exec" {
    command = "ssh -i /home/ubuntu/keys/To_Be_Deleted_Singapore.pem ${data.aws_instance.client_terraform.private_ip} 'bash -s' < /home/ubuntu/install_scripts/traefik.sh"
  }
}
